...
def errors(songNum, failedSongs, driver):

    while songNum <= 1000:
        webpage = r"http://www.ergaran.in/2016/01/" + str(songNum) + ".html"
        try:
            print(driver.find_element_by_class_name("post.hentry.uncustomized-post-template").text)
        except:
...
while songNum > 659 and songNum <= 940:
        try:
            ...
            print("On song number: " + str(songNum))
            f = open("D:/Ergaran Web Word Songs/"+ str(songNum) + ".txt", 'w', encoding='utf_8')
            f.write(driver.find_element_by_class_name("post.hentry.uncustomized-post-template").text + "HELLO")
            f.close()
...
failed = open("D:/Ergaran Web Word Songs/FailedSongs.txt",'w', encoding='utf_8')
