...
while songNum < 661:
    try:
        ...
        font.name = 'Arial'
        font.size = Pt(22)
        doc.save("D:/Ergaran Web Word Songs/"+ str(songNum) + ".docx")
        # f.close
    except:
...
while songNum < 941:
    try:
        ...
        # f = open("D:/Ergaran Web Word Songs/"+ str(songNum) + ".txt", 'w', encoding='utf_8')
        print(borsht.find(class_ = 'post hentry uncustomized-post-template').get_text(" \n ", strip=True)) ## it works!!!
        text = borsht.find(class_ = 'post hentry uncustomized-post-template').get_text(" \n ", strip=True)## that way I don't just have a bunch of open files lol ##this is after the print just incase the file is not found,
        # f.write(text)
        doc = docx.Document()
        doc.add_paragraph(text)
        font = doc.styles['Normal'].font
